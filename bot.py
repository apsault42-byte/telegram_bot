...
# COMPLETION CHECKER
# ======================================================================
def check_offer_completed(check_link: str) -> tuple[bool | None, str]:
    """
    Fetch the check link and look for completion keywords.
    Returns: (status, debug_text)
        status: True = completed, False = not completed, None = error
        debug_text: First 300 chars of response for debugging
    """
    headers = {
        "User-Agent": USER_AGENT,
    }
    ...
    debug_text = text[:300] if text else "(empty response)"

    # Check keywords
    text_lower = text.lower()
    for keyword in COMPLETION_KEYWORDS:
        if keyword.lower() in text_lower:
            return True, debug_text
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        ...
        completed, debug_text = await asyncio.to_thread(check_offer_completed, check_link)
        ...
        if completed is None:
            print(f"Chat {chat_id} | Attempt {attempt} | Check link error: {debug_text}")
        else:
            # Show debug text on first attempt and every 5 attempts
            if attempt == 1 or attempt % 5 == 0:
                print(f"Chat {chat_id} | Attempt {attempt} | Not completed. Response: {debug_text[:200]}")
